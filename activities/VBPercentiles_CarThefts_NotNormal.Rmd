...
Random sampling usally attracts an average given the nature of it's processes. It's difficult to get an accurate random sample because impicent bias always comes into account with any population. For example trying to achieve random sampling by calling home phones is skewed because older folks are more active. Also if you try to sample those with boats you will  attract a wealthier population. 

Reports should  acknowledge the potential bias within the sampling and be transparent. Reporters should avoid data where bias is blatantly affecting the results of the story. If a story reports that most people who use boats also eat the most lobster then you might recognize the wealth bias and you should mention so in the story.
...
Maryland has 223 aces, thus they are in the 95th percentile. This means they have achived more aces then 94 percent of all teams.
...
spotify |> 
 ggplot() + 
  ...
  geom_vline(aes(xintercept = mean(danceability) - sd(danceability)), 
...
The distribution is skewed to the left and 68 percent of the data falls within the line, which is the first standard deviation. 
