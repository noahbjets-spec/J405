...
Voter turnout seems to be strongly linked to income and education. Both seem to show positive correlations. White population has a somewhat negitive correlation and unemployment and median age don't seem to relate to turn out at all. 
...
The model shows that income and education are positive steady and statistically significant. "Percent white" has a negitive effect. 
The residuals are centered around zero and have a small spread. This means the model fits well. The R squared is high, which means the model explains most of the differences in turnout, and is useful for our study. 
...
Model 2 and 4 have adjusted R-squared values close to the full model. It means they explain turnout almost as well. Model 3 performs  worst because it removes too many variables.

```{r}
# Model 2
model2 <- lm(turnout ~ income + pct_bachelors_higher + pct_white, data = md_turnout)


# Model 3
model3 <- lm(turnout ~ income + pct_bachelors_higher, data = md_turnout)


# Model 4
model4 <- lm(turnout ~ income + pct_bachelors_higher + pct_white + population, data = md_turnout)
...
The analysis shows which county traits are linked to higher voter turnout. By knowing that places with higher income and education levels tend to vote more, I will focus on why those communities participate more and why others might be falling behind in my reporting for the story. 