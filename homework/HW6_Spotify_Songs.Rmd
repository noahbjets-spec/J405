...
```{r}
spot_mean <- mean_dance <- mean(spotify$danceability, na.rm = TRUE)
...
spotify |> 
  ggplot() + 
  geom_histogram(aes(x = danceability), binwidth = 0.30)
...
```{r}
spotify |> ggplot() + geom_histogram(aes(x = danceability), binwidth = 0.05) + geom_vline(aes(xintercept = mean(danceability)), color = "red", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(danceability) - sd(danceability)), color = "blue", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(danceability) + sd(danceability)), color = "blue", linetype = "dashed", size = 1)
```
...
The graph shows that most songs on Spotify are upbeat and have a danceable tempo. The data is slightly left-skewed, with the mean centered around 0.65. The standard distribution indicates that most songs usually fall in the 0.5–0.85 range, with most hovering around 0.7. 

Music meant to move 
Spotify research data indicates an upward trend in songs classified as dance-worthy. More and more, artists seem to release music designed to get listeners on their feet, rather than tracks meant for sitting back and relaxing.
